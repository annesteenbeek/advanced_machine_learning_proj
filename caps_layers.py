...
def conv_caps_layer(input_layer, capsules_size, nb_filters, kernel_size, stride=2):
    """
        Capsule layer for the convolutional inputs
        **input:
            *input_layer: (Tensor)
            *capsule_size: (Integer) the number of capsule in this layer.
            *kernel_size: (Integer) Size of the kernel_size for each filter.
    ...
    capsules = tf.contrib.layers.conv2d(input_layer,
                                        nb_filters * capsules_size,
                                        kernel_size, 
                                        stride,
                                        padding="VALID")
    # Also possible to use ReLU activation function on primary caps
    # TODO: test relu, add activation_fn=tf.nn.relu to capsule

    # conv shape: [?, kernel_size, kernel_size, nb_filters]
...
def fully_connected_caps_layer(input_layer, capsules_size, nb_capsules, iterations=4):
    ...
    # The tensorflow varriable that gets trained using backtracking
...
def squash(vector):
    """
        ...
        **input: **
            *vector
    """
    vector += 1e-9 # Workaround for the squashing function ...
